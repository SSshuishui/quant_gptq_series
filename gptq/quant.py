...
def quantize_spqr(x, scale, zero, maxq, eps=1e-9):
    q = torch.clamp(torch.round(x / scale.clamp_min(eps) + zero), 0, maxq)
    return q

def dequantize_spqr(x, scale, zero, eps=1e-9):
    return scale * (x - zero)

def quantize_dequantize_spqr(x, scale, zero, maxq, eps=1e-9):
    q = torch.clamp(torch.round(x / scale.clamp_min(eps) + zero), 0, maxq)
    return scale * (q - zero)
...
class Quantizer(nn.Module):
    ...
    def configure(self, bits, perchannel=False, sym=True,
            mse=False, norm=2.4, grid=100, maxshrink=.8, trits=False, beta=1.0
    ):
...
class SpQRQuantizer(nn.Module):
    ...
    def find_params(self, x, weight=False):
        ...
        if self.qq_scale_bits is not None:
            ...
            self.quant_scale = self.qq_scale.quantize_spqr(scale_groups)
            self.scale = self.qq_scale.dequantize_spqr(self.quant_scale).reshape_as(self.scale)
        ...
        if self.qq_zero_bits is not None and ((not self.round_zero) or self.qq_zero_bits < self.bits):
            ...
            self.quant_zero = self.qq_zero.quantize_spqr(zero_groups)
            self.zero = self.qq_zero.dequantize_spqr(self.quant_zero).reshape_as(self.zero)
    ...
    def quantize_dequantize_spqr(self, x):
        if self.ready():
            return quantize_dequantize_spqr(x, self.scale, self.zero, self.maxq)
        return x

    def quantize_spqr(self, x):
        if self.ready():
            return quantize_spqr(x, self.scale, self.zero, self.maxq)
        return x

    def dequantize_spqr(self, x):
        if self.ready():
            return dequantize_spqr(x, self.scale, self.zero)
        return x
