...
from .quant import SpQRQuantizer, dequantize_spqr, quantize_spqr
        ...
        **kwargs,
        ...
        for block_start in block_start_iter:
            ...
            for column_index in range(block_start, block_end):
                ...
                weight_quant_i = quantize_spqr(
                    weight[:, column_index].unsqueeze(1), quantizer.scale, quantizer.zero, quantizer.maxq
                )
                weight_i_quantized = dequantize_spqr(weight_quant_i, quantizer.scale, quantizer.zero).reshape_as(
                    weight[:, column_index]
                )
                ...
                if unstructured_outlier_threshold != float("inf"):
                    ...
                    weight_i_quantized_wo_outliers = dequantize_spqr(
...
def get_leave_one_out_error(group_weight: torch.Tensor, group_diag_hessian_inv_cho: torch.Tensor, *, bits, sym):
    ...
    fast_quantizer = SpQRQuantizer(shape=groupwise_loo_data.flatten(0, 1).shape)
    ...
    loo_groupwise_reconstructed_weights = fast_quantizer.quantize_dequantize_spqr(
    ...
    base_quantizer = SpQRQuantizer(shape=group_weight.shape)
    base_quantizer.configure(bits, perchannel=True, sym=sym)
    base_quantizer.find_params(group_weight, weight=True)
    baseline_reconstructed_weights = base_quantizer.quantize_dequantize_spqr(group_weight)
    baseline_errors_sq = (
